...
### Breaking — `/health` queue field renamed `depth` → `jobs_total`

- **`GET /health` response: `queue.depth` is now `queue.jobs_total`.** Introduced in the queue-aware speak() rework (commit `65cf1b5`, PR #129). Callers that read `response.queue.depth` to determine backlog size must update to `response.queue.jobs_total`. The sibling field `queue.active_jobs` (count of currently-generating jobs) is unchanged. Dashboard integrations, monitoring scripts, and any client that inspects the `/health` JSON payload are affected.
